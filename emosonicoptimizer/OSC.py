...
----------
v?.?.?	- 01. Jun 2012
	Change by Thomas Hermann: sending bundles with timetags as from time.time()
	did not work properly. I added in OSCTimeTag the nr. of seconds from 1900 to 1970
	and multiplied by a factor for seconds to OSC seconds format.
...
def OSCTimeTag(time):
	...
	"""
	if time > 0:
		# changed by Thomas Hermann: uncommented next two (original) lines
		# fract, secs = math.modf(time)
		# binary = struct.pack('>ll', long(secs), long(fract * 1e9))
		# changed by Thomas Hermann: added following two lines 
		# to correct a problem with bundle timetags for SC3
		# kSECONDS_FROM_1900_to_1970 = 2208988800L
		# kSecToOSC = 4294967296	
		binary = struct.pack(">Q", long( (time + 2208988800L)*4294967296))
	else:
